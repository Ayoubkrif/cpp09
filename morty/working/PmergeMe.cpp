...
PmergeMe::PmergeMe(int ac, char** av) : _vecTime(0), _dequeTime(0)
{
...
void PmergeMe::run()
{
	...
	std::deque<int> tmp_deque = _deque;
	start = clock();
	tmp_deque = sortDeque(tmp_deque);
	end = clock();
...
void PmergeMe::printTimes()
{
	...
	<< "Time to process a range of " << _vec.size() 
	<< " elements with std::deque<>  : " << _dequeTime * 1000 << " ms" << std::endl; 
}
...
std::vector<int> PmergeMe::sortVector(std::vector<int>& tmp_vec)
{
	...
	std::vector<size_t> jacobliste = generateJacobsthalList(pairs.size());
	for (size_t i = 0; i < pairs.size(); i++)
	{
		...
		std::vector<int>::iterator it = std::lower_bound(sortedGrands.begin(), sortedGrands.begin() + big, small);
		sortedGrands.insert(it, small);
	}

	if (hasUnpaired)
	{
		std::vector<int>::iterator it = std::lower_bound(sortedGrands.begin(), sortedGrands.end(), unpaired);
		sortedGrands.insert(it, unpaired);
	}
	return sortedGrands;
}

std::deque<int> 	PmergeMe::sortDeque(std::deque<int>& tmp_deque)
{
	if (tmp_deque.size() <= 1)
		return tmp_deque;
	...
	int unpaired 						= -1;
	bool hasUnpaired 					= (tmp_deque.size() % 2 != 0);

	for (size_t i = 0; i + 1 < tmp_deque.size(); i += 2)
	{
	...
	if (hasUnpaired)
		unpaired = tmp_deque.back();
	for (size_t i = 0; i < pairs.size(); i++)
		grands.push_back(pairs[i].first);
	std::deque<int> sortedGrands = sortDeque(grands);
	...
	for (size_t i = 0; i < pairs.size(); i++)
	{
		size_t big = std::lower_bound(sortedGrands.begin(), sortedGrands.end(), pairs[jacobliste[i]].first) - sortedGrands.begin();
		int small = pairs[jacobliste[i]].second;
		std::deque<int>::iterator it = std::lower_bound(sortedGrands.begin(), sortedGrands.begin() + big, small);
		sortedGrands.insert(it, small);
	}

	if (hasUnpaired)
	{
		std::deque<int>::iterator it = std::lower_bound(sortedGrands.begin(), sortedGrands.end(), unpaired);
		sortedGrands.insert(it, unpaired);
	}
	return sortedGrands;
}
